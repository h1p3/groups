...
## 16. Фикс: «attract в 15 раз медленнее exclude» — ВЫПОЛНЕНО `[x]`

Подробности: ARCHITECTURE.md §13. Профилирование на реальной модели нашло
причину не в подмешивании, а в одной строке, общей для exclude и attract:
`self.llm.eval_logits[-1]` (свойство llama-cpp-python) конвертирует **весь**
`scores[:n_tokens, :]` в Python-список на каждый токен — растущая с длиной
ответа стоимость, замерено ~104мс/вызов против ~6мс на сам forward pass
(17x переплата за чтение уже готового результата).

- [x] Фикс (`engine/llamacpp.py::_generate_filtered`): прямой доступ
      `self.llm.scores[self.llm.n_tokens - 1]` (уже numpy-массив) вместо
      `self.llm.eval_logits[-1]`. Побитово идентично (`np.allclose`),
      170 → 0.5мс на чтение, **160x** ускорение самой операции.
- [x] Замерено на реальной модели (~60 токенов, кэшированный `VocabIndex`,
      без отказов guard'а): exclude 6.13с→1.27с (4.8x), attract
      11.50с→0.68с (16.9x). Соотношение attract/exclude ушло из диапазона
      «в разы медленнее» практически полностью.
- [x] Объяснение, почему выглядело как «attract специфично медленный»: оба
      режима платят эту цену на каждый токен, но attract/include обычно шёл
      через `generate_guarded` (чанками, §13 из предыдущей нумерации — Include
      ↔ retrieval), а exclude — одним вызовом `engine.generate()`; больше
      чанков — больше раз пересчитана одна и та же дорогая операция. Не
      attract был медленным, оба были одинаково медленными на токен.
- [ ] **Не тронуто:** `logits_all=True` в конструкторе `Llama(...)` теперь
      избыточен (нигде в коде `eval_logits` больше не используется), но
      оставлен — может понадобиться для нереализованной «Real-time logits
      visualization» (§12 старой нумерации/§9 текущей ARCHITECTURE.md).
- [ ] **Остаётся отдельной, но теперь куда менее заметной проблемой:**
